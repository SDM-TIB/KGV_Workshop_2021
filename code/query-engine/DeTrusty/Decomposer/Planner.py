    ...
    def includePhysicalOperators(self, tree):
        if isinstance(tree, Leaf):
            if isinstance(tree.service, Service):
                if tree.filters == []:
                    return IndependentOperator(self.query, tree, self.contact, self.config, self.val_config)
                else:
                    n = IndependentOperator(self.query, tree, self.contact, self.config, self.val_config)
    ...
    def includePhysicalOperatorJoin(self, l, r):
        ...
        if isinstance(l, IndependentOperator) and isinstance(l.tree.service.triples, list) and l.tree.service.triples[0].subject.constant:
            if len(join_variables) > 0:
                n = TreePlan(JOIN_TO_USE(join_variables), all_variables, l, r)
                dependent_join = True
            ...
            if len(join_variables) > 0:
                n = TreePlan(JOIN_TO_USE(join_variables), all_variables, r, l)
                dependent_join = True
...
class IndependentOperator(object):
    ...
    def execute(self, outputqueue, processqueue=Queue()):
        ...
        p = Process(target=self.contact, args=(self.server, self.query_str, outputqueue, self.val_config, self.tree.service.target_shape, self.buffersize, self.tree.service.limit))
        p.start()
