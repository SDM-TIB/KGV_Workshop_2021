...
class Decomposer(object):
    ...
    def decomposeBGP(self, tl):
        ...
        for s in res:
            if len(res[s]) == 1:
                if len(self.config.metadata[res[s][0]]['wrappers']) == 1:
                    ...
                    target_shape = self.config.metadata[res[s][0]]['target_shape']
        ...
        if qpl0 and not self.joinlocally:
            ...
            qpl0 = []
            for e in joins:
                # -- join molecules in same source; if joinstarslocally=False --
                endp = e.split('|')[0]
    ...
    def metawrapperdecomposer(self, res, triplepatterns):
        ...
        if len(sourceindex) == 1:
            # -- several molecules but all in one source
            ...
            if len(singlesource) > 0:
                if len(singlesource) == 1:
                    for url in singlesource:
                        # -- several molecules, several sources (intersection of predicates), only one answers all triple patterns --
                        servs.append(Service("<" + url + ">", list(set(sourceindex[url]))))
                else:
                    for url in singlesource:
                        # -- several molecules, several sources (intersection of predicates) valid --
                ...
                for url in sourceindex:
                    if len(sourceindex[url]) == len(triplepatterns):
                        # -- several molecules, several sources (intersection of predicates), all triple patterns --
                        servs.append(Service("<" + url + ">", list(set(sourceindex[url]))))
                        ignore.append(url)
                    else:
                        # -- several molecules, several sources (intersection of predicates), all intersecting triple patterns --
                        joins.append(JoinBlock([Service("<" + url + ">", list(intersects))]))
                for url in sourceindex:
                    if len(sourceindex[url]) > 0 and url not in ignore:
                        # -- several molecules, several sources (intersection of predicates), all non-intersecting triple patterns --
                        servs.append(Service("<" + url + ">", list(set(sourceindex[url]))))
        else:
            # TODO: check other decompositions to make a true union
            # -- several molecules, several sources, empty intersection
