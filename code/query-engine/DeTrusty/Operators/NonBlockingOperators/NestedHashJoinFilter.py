...
# Special rewritting in FILTER Clause for DBpedia Endpoint
LEFT_BRACKET_VAR = 'str(?'
RIGHT_BRACKET_VAR = ')'

LEFT_BRACKET_URI = '"'
RIGHT_BRACKET_URI = '"'

# Normal FILTER Clauses
# LEFT_BRACKET_VAR = '?'
# RIGHT_BRACKET_VAR = ''
            ...
            for tuple in filter_bag:
                ...
                for var in self.vars:
                    ...
                    if v.find("http") == 0:  # uris must be passed between < .. >
                        v = LEFT_BRACKET_URI + v + RIGHT_BRACKET_URI
                        v = LEFT_BRACKET_VAR + var + RIGHT_BRACKET_VAR + "=" + v
                        and_expr.append(v)
                    else:
                        if '^^<' not in v:
                            v = '"' + v + '"'
                            vf = LEFT_BRACKET_VAR + var + RIGHT_BRACKET_VAR +  "=" + v  # + " || " + "?" + var + "=" + v + "^^<http://www.w3.org/2001/XMLSchema#string>)"
                            and_expr.append(vf)
                        else:
                            loc = v.find('^^<')
                            vf = '"' + v[:loc] + '"' + v[loc:]
                            v = "(" + LEFT_BRACKET_VAR + var + RIGHT_BRACKET_VAR + "=" + vf + ' || ' + LEFT_BRACKET_VAR + var + RIGHT_BRACKET_VAR + '="' + v[:loc] + '")'
                            and_expr.append(v)
                if len(and_expr) > 1:
